...
public class Main1 {  
	...
	public static void main(String[] args) throws Exception {  
       ...
       final int THREAD_NUMBERS=4;
        ...
        //把消费者的位置信息引用注入到生产者 只有一个消费者的情况可以省略
        ringBuffer.addGatingSequences(transProcessor.getSequence());  
          
        //把消息处理器提交到线程池  
        executors.submit(transProcessor);

        //如果存在多个消费者 那重复执行上面3行代码 把TradeHandler消费者类换成其它消费者类

        Future<?> future= executors.submit((Callable<Void>) () -> {
            long seq;
            for(int i=0;i<10;i++){
                seq = ringBuffer.next();//占个位置 --ringBuffer一个可用区块
                ringBuffer.get(seq).setPrice(Math.random()*9999);//给这个区块放入 数据 生产者
                ringBuffer.publish(seq);//发布这个区块的数据使handler(consumer)可见去消费
            }
            return null;
        });
        future.get();//等待生产者结束
